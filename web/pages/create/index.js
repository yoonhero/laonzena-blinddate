...
export default function Login() {
    ...
    const loginRequest = async () => {
        ...
        if (phoneNumber == "") {
            setNotification("전화번호를 입력해주세요!");
            return;
        }

        const [status, message] = await CreateUserAPI({ schoolNumber, password, phoneNumber });
            ...
            <main className='w-screen min-h-screen bg-gray-100'>
                ...
                <div className='w-full h-[80vh] relative flex flex-col items-center justify-center'>
                    ...
                    <form className='flex flex-col items-center justify-center'>
                        <div className='m-3 flex items-center ' key={1}>
                            <input
                                onChange={(e) => setSchoolNumber(e.target.value)}
                                value={schoolNumber}
                                className={`appearance-none bg-transparent text-2xl font-semibold  border-b-2 border-gray-300 transform transition east-in-out duration-300  w-full text-gray-700 mr-3 py-3 px-5 leading-tight focus:outline-none ${
                        ...
                        <div className='m-3 flex items-center' key={2}>
                            <input
                                ...
                                value={password}
                                className='appearance-none bg-transparent text-2xl font-semibold  border-b-2 border-gray-300 transform transition east-in-out duration-300  w-full text-gray-700 mr-3 py-3 px-5 leading-tight focus:border-teal-500 focus:outline-none'
                                type='text'
                                placeholder='비밀번호'
                                aria-label='Password'
                            />
                        ...
                        <div className='m-3 flex items-center' key={4}>
                            <input
                                onChange={(e) => setOnlySema(e.target.value)}
                                value={onlySema}
                                className={`appearance-none bg-transparent text-md md:text-xl font-semibold  border-b-2 border-gray-300 transform transition east-in-out duration-300  w-full text-gray-700 mr-3 py-2 px-2 md:px-2 leading-tight focus:border-red-500 focus:outline-none text-center`}
                                type='text'
